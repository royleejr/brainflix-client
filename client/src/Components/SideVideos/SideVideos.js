...
class SideVideos extends React.Component {
  ...
  getSideVideoData = () => {
    Axios.get("https://roy-brainflix.herokuapp.com/videos").then((response) => {
      this.setState({
  ...
  render() {
    const newSideVideoData = this.state.sideVideoData.filter(
      (item) => item.id !== this.props.mainVideoData.id
    );
    return (
      <aside className="side-videos">
        ...
        {newSideVideoData.map((item) => {
          return (
            <Link
              to={`/videos/${item.id}`}
              className="side-videos__video-container"
              key={item.id}
            >
              <video
                className="side-videos__video-container__video"
                poster={item.image}
              ></video>
              <div className="side-videos__video-container__info-div">
                <p className="video-container__info-div__title">{item.title}</p>
                <p className="video-container__info-div__channel">
                  {item.channel}
                </p>
              </div>
            </Link>
